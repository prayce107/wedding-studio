(function () {
  ...
  const TemplateAdapter = {
    render(dom, data) {
      ...
      // 7. Dynamic Blossom Floating Petals
      const blossomContainer = dom.getElementById("blossom-container");
      if (blossomContainer && window.KoreanBlossomAnimation) {
        window.KoreanBlossomAnimation.init(blossomContainer, decoration);
      }
      ...
      if (galleryEl) {
        ...
        galleryEl.innerHTML = albumItems.length ? albumItems.map((x, i) => `
          <figure data-src="${esc(x.src)}">
            ...
            <figcaption>${esc(x.caption || "Sweet Memory")}</figcaption>
          </figure>
