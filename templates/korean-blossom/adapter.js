(function () {
  ...
  const TemplateAdapter = {
    render(dom, data) {
      ...
      // 8. Accommodation (Where to Stay) Hotel Cards Binds
      const hotelsContainer = dom.getElementById("hotelsGrid");
      if (hotelsContainer) {
        const hotels = data.accommodation?.hotels || [
          { name: "The Blossom Boutique Stay", desc: "Resort modern dengan nuansa pastel romantis.", address: "Jl. Bunga Indah No. 5 (3 menit ke lokasi)", link: "https://booking.com" },
          { name: "Seoul Garden Inn", desc: "Hotel premium dekat lokasi strategis pernikahan.", address: "Jl. Taman Raya No. 12 (8 menit ke lokasi)", link: "https://booking.com" }
        ];
      ...
      if (attireContainer) {
        const colors = data.attire?.colors || ["#FFF0F2", "#FFE4E1", "#E6E6FA", "#4A3E3F"];
      ...
      if (streamingScreen) {
        let videoFrame = dom.getElementById("koreanVideoFrame");
        ...
        if (videoUrl) {
          ...
          if (!videoFrame) {
            ...
            videoFrame.id = "koreanVideoFrame";
            videoFrame.setAttribute("allowfullscreen", "true");
            videoFrame.style.width = "100%";
            videoFrame.style.aspectRatio = "16/9";
            videoFrame.style.border = "1px solid rgba(230, 160, 180, 0.4)";
      ...
      if (data.style && data.style.colors) {
        ...
        root.style.setProperty('--primary', data.style.colors.primary || '#4A3E3F');
        root.style.setProperty('--accent', data.style.colors.secondary || '#FFB6C1');
        root.style.setProperty('--bg', data.style.colors.background || '#FFF0F2');
      }
