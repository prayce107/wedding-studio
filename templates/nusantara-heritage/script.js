(() => {
  ...
  if (openBtn && cover) {
    openBtn.addEventListener("click", () => {
      ...
      // Animate seal zoom & rotate
      const seal = document.querySelector(".seal-wax-stamp");
      if (seal) {
        seal.style.transform = "scale(1.3) rotate(35deg)";
      }
    ...
    rsvpForm.addEventListener("submit", async e => {
      ...
      try {
        if (window.rsvpService) {
          ...
          const r = JSON.parse(localStorage.getItem("nusantaraHeritageRSVP") || "[]");
          r.unshift({ ...item, time: new Date().toLocaleString("id-ID") });
          localStorage.setItem("nusantaraHeritageRSVP", JSON.stringify(r));
        }
        ...
        if (statusEl) {
          statusEl.innerHTML = `
            <div class="success-bubble">
              ...
              <p style="margin-top: 5px; color: var(--accent-light);">✔ Konfirmasi RSVP berhasil dikirim. Terima kasih!</p>
            </div>
          `;
        }
        ...
        if (message) {
          try {
            const wishes = JSON.parse(localStorage.getItem("nusantaraHeritageGuestbook") || "[]");
            wishes.unshift({ n: guestName, w: message, time: new Date().toLocaleString("id-ID") });
            localStorage.setItem("nusantaraHeritageGuestbook", JSON.stringify(wishes));
            renderWishes();
          } catch (e) {
